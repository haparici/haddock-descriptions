...
Finally, the last term in equation \ref{pragmatic-listener} samples contexts in models that incorporate Context Coordination, as shown in \ref{context-prior}. [\textbf{This needs to be completed}]
...
\begin{equation}
\begin{multlined}
  P(C) =  \frac{1}{Z}2^{\alpha \mid C\mid}\text{, where Z} =\sum_{C \in \mathcal{P}(C)}2^{\alpha \mid C\mid}\text{    , such that} \\
  P(C) =
  \begin{cases}
    \text{Uniform for } \alpha = 1\\
    \text{For } \alpha > 1\text{: Bigger contexts receive higher probability in the prior.}
  \end{cases}
\end{multlined}
  \label{context-prior}
\end{equation}
