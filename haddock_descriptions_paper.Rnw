...
Possible threshold values are obtained by extracting the referents in the display that are in the extension of the adjectivally modified noun (e.g., bag), which is taken to provide the Comparison Class against which the adjective is evaluated.
This is done to ensure that the sizes of other objects present in the context that are not in the extension of the nested noun are not included as possible threshold values. 
the intuition that attributive adjectives NP sister the Comparison Class.
This manipulation derives dispreference for contexts with bigger bag.
For instance, in a context with two bags (see Figure \ref{2bag-context}), two possible thresholds would result This prior has an important role in deriving the.
Bag resolution for contexts 1 and 3 involves three possible thresholds. Bag resolution for context 2 involves xxx. Each threshold receives higher probability in the prior in context 2 compared to contexts 1 and 3.
