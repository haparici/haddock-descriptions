...
The singularities of Haddock Descriptions with respect to uniqueness requirements renders them ambiguous between two readings: a relative and an absolute one. These two readings can be clearly teased apart when the second NP of a Haddock Description is modified by a superlative adjective as in (\ref{superlative}).

\begin{exe}
\ex\label{superlative} The rabbit in the biggest bag.
\end{exe}

On its absolute reading, sentence (\ref{superlative}) is true iff the context contains a unique rabbit that is in the unique biggest bag. On its relative reading, the sentence is true as long as there is a unique rabbit that is inside the biggest rabbit-containing bag, even if the context contains bigger rabbitless bags. 
...
The case of positive form gradable adjective is different, as these predicates have been argued to be context-sensitive such that their interpretation involves resolving an adjectival threshold with respect to a contextually salient Comparison Class determined by the head noun. 
...
Our results show that referent choice is modulated by referents that are incompatible with the available linguistic input (i.e., bags that do not contain rabbits in them when participants opted for the bag resolution) for positive but not for comparative form adjectives.
