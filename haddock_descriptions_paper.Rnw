...
execute_model <- function(randomVariable, cond, model, context, defArt, package) {
  ...
  model_data <- list(randomVariable, visuals, descs, costs, context, defArt)
...
test_result <- execute_model(pos, 1, "haddock_model.wppl","no-cc","bumford",".")
...
for(defArtMeaning in defArtMeanings) { 
  for (context in contexts) {
    for (randomVariable in randomVariables) {
      for (cond in conds) {
        for (model in models) {
          result <- execute_model(randomVariable, cond, model, context, defArtMeaning)
...
\noindent
1. Posterior probabilities go down if we have both pos and comparative alternative utterenaces compete with each other (see model results in section 4.4.). \\
\noindent
...
bags.standard<-subset(results, Container=="bag" & posMeaning=="standard" & Model=="haddock_model.wppl")
...
\noindent
\textbf{Open Questions:} I do not understand the compartive results with standard meaning. Why are Contexts 1 and 2 not the same? This needs to be further investigated.
...
## Plots ##
bags.bumford<-subset(results, Container=="bag" & posMeaning=="bumford" & Model=="haddock_model.wppl")
...
bags.standard.pragList2<-subset(results, Container=="bag" & posMeaning=="standard" & Model=="haddock_model_pragmaticListener2.wppl")
...
## Plots ##
bags.bumford.pragList2<-subset(results, Container=="bag" & posMeaning=="bumford" & Model=="haddock_model_pragmaticListener2.wppl")
