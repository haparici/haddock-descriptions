...
gamma <- .5
#higher cost coefficient increases informativity effect but 
costCoefficient <- 1
...
The question we address here is whether the externally-imposed constraints on a  referent have constrain the interpretation of gradable adjectives characterizing that referent, as the incremental context-narrowing hypothesis predicts.
...
Do external constraints (e.g.\ containing a rabbit) constrain comparison classes for adjectives as well?
...
We present simulation results that further explore semantic and pragmatic accounts of Haddock descriptions that do not involve incremental context narrowing.
...
bags.standard<-subset(results, Container=="bag" & DefArtMeaning=="standard" & DefArtMeaning=="standard" & Model=="haddock_model.wppl", ListenerLevel=="1")
