...
#test_result <- execute_model(pos, 1, "haddock_model.wppl","no-cc","bumford",".",gamma,costCoefficient,highScopeConstrualProb,1)
...
The semantic and the pragmatic accounts described so far differ in that modified Haddock Descriptions with a comparative adjective or a superlative are ambiguous between a relative and an absolute reading.
...
The question we address here is whether the externally-imposed constraints on a referent constrain the interpretation of gradable adjectives characterizing that referent.
When a gradable adjective modifies a noun, the noun constrains the comparison class ({\em tall building} vs.\ {\em tall stack of paper}), so description-internal constraints on the discourse referent clearly restrict the comparison class for a gradable adjective. 
Do external constraints (e.g.\ containing a rabbit) constrain comparison classes for adjectives as well? This would correspond to the high scope reading.
...
This discussion gets murked because absolute readings are confounded with informativity, and because the following is possible:

Note that in the case of adjectives, contextual variables: you can have low scope for adjective and still result in a relative-like interpretation. Here we seek to tease the contribution of these two components apart.
We seek to tease apart whether the apparent relative reading results from true scopal ambiguities or through context-sensitivity.
...
Our results show effects of referents that are incompatible with the available linguistic input.
This argues against an incremental context-narrowing account of Haddock descriptions, and has implications for alternative approaches as well.
We present simulation results that further explore semantic and pragmatic accounts of Haddock descriptions that do not involve incremental context narrowing, particularly with respect to the scopal preferences of the comparative vs. the positive form adjective. We show that, at least when part of a description, comparatives prefer to be interpreted high, while postive form adjectives prefer to be interpreted low.
