...
#setwd("/Users/elizabeth/Documents/GitHub/haddock-descriptions")
...
Readings were the uniqueness requirement of the nesded DP seems to be relaxed are referred to as {\em relative readings}, in contrast to {\em absolute readings}, where uniqueness is satisfied for both determiners. The clearest empirical point for the existence Relative and absolute readings can be teased apart when the nested NP of a Haddock Description is modified by a superlative adjective as in (\ref{superlative}).
...
Positive and Comparative form adjectives resemble superlatives in that they are also interpreted w.r.t. a CC. However, these three types of predicates also differ in fundamental ways. Firs, the threshold of a pos form adjective is not semantically bound. Interpreted w.r.t. context:\footnote{Here we use POS but could also be using typeshifting for this.} CC argument semantically provided?
...
data<-read.csv("matrix-pilot.csv", header=TRUE) #225 participants
...
While our results rule out the high-scope interpretation of postive form adjectives, they leave open the question of what precise mechanisms underlie the observed patterns of results. 
...
\begin{figure}[h]
\centering
<<Plot-standard-poslow, echo=FALSE, cache=TRUE, warning=FALSE, message=FALSE, fig.width = 6, fig.height=3.5>>=
...
myColors <- c("#56B4E9","lightblue","#F0E442","#C44D00")
...
@
\caption{Simulation results for model using standard meaning (L1) and low scope for positive morpheme. Left: Allow uninformative thresholds. Right: Disallow.}
...
df.standardCmpLow<-subset(results, Adjective=="bigger" & DefArtMeaning=="standard" & Model=="haddock_model.wppl"  & ListenerLevel==1)
#Should we pick an arbitrary value for "allow uninformative thresholds"? It's n/a.
#Otherwise we are combining both runs.
...
plot.standardCmpLow <- ggplot(df.standardCmpLow, aes(x=Condition, y=Probability, fill=Container)) + 
  ...
  xlab("Display Type") +
  ylab("Probability of Referent Choice") +
  ggtitle("Standard-the + bigger","Pragmatic L1") +
  facet_wrap(~Context)
...
df.bumfordCmp<-subset(results, Adjective=="bigger" & DefArtMeaning=="bumford" & Model=="haddock_model.wppl" & ListenerLevel==1)

plot.bumfordCmp<-ggplot(df.bumfordCmp, aes(x=Condition, y=Probability, fill=Container)) + 
  geom_bar(position="fill", stat="identity") +
  fillScale +
  theme_bw() +
  theme(axis.text.x = element_text(size=8),
  ...
  xlab("Display Type") +
  ylab("Probability of Referent Choice") +
  ggtitle("Bumford-the + bigger","Pragmatic L1, 50% chance of high comparative scope") +
    facet_wrap(~Context)
