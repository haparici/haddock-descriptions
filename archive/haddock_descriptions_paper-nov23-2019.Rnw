...
\usepackage{amsmath,amsfonts, amssymb,latexsym,mathrsfs, fancyhdr,theorem,  pifont, setspace, verbatim,  qtree, lscape, tipa,  hyperref,verbatimbox, wasysym, natbib,soul, minibox, lipsum, amssymb, color, multirow, multicol, soul,geometry,graphicx, wrapfig,gb4e,booktabs,stmaryrd}
...
test_result <- execute_model(pos, 1, "haddock_model.wppl","cc","standard",".",gamma,costCoefficient,highScopeConstrualProb,1)
      ...
      for (randomVariable in randomVariables) {
        for (cond in conds) {
          for (model in models) {
            ...
            result <- execute_model(randomVariable, cond, model, context, defArtMeaning,".",gamma,costCoefficient,highScopeConstrualProb,pragListenerLevel)
...
Haddock proposed a solution based on the assumption that semantic interpretation proceeds {\em incrementally}, i.e., from left to right. 
In particular, the conceives of the task of identifying a referent for a description as a constraint satisfaction problem in which constraints are added incrementally as the sequence of words unfolds. 
For example, after hearing {\em the rabbit}, 
the constraint '$\textsf{rabbit}(x)$' is added; 
after hearing {\em in}, the constraint '$\textsf{in}(x,y)$' is added; 
{\em the hat} further constrains $y$ to be a hat. 
On this view, the uniqueness requirement of the definite article is merely that there is a maximum of one individual who can satisfy {\em all} of the constraints on $y$, 
including that it contains a rabbit.

The question we ask here is: If definiteness for a given noun phrase is calculated by taking into consideration only those referents that satisfy {\em all} of the constraints established so far, are comparison classes for gradable modifiers restricted in the same way?
In other words, if a gradable modifier is used by a speaker in order to help a listener identify a referent, is the comparison class restricted to only those referents that could possibly satisfy the constraints on the referent established so far?


As the description is interpreted incrementally, the listener narrows down the set of potential referents in the context to those individuals that are compatible with the linguistic information available at any given time. 
...
We therefore have different thresholds for different types of objects. 
...
\caption{Literal Listener model outputs for standard meaning and no-cc}
\label{litlist-standard}
\end{figure}

\begin{figure}
\includegraphics[width=\linewidth]{litlist-standard.pdf}
\caption{Literal Listener model outputs for Bumford's meaning and no-cc}
...
\noindent
It seems that the probabilities go down if thetas include n+1\\
